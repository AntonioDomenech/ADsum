"""Helpers for enumerating audio devices using FFmpeg."""
...
def list_input_devices() -> List[DeviceInfo]:
    """Return an empty list because legacy backends are no longer available."""

    LOGGER.debug("Legacy sounddevice backend removed; skipping device enumeration.")
    return []
...
def format_device_table(devices: Optional[Iterable[DeviceInfo]] = None) -> str:
    ...
    if backend not in {"", "ffmpeg", "none"}:
        return (
            f"Audio backend '{backend}' is no longer supported. "
            "Set ADSUM_AUDIO_BACKEND=ffmpeg and provide an FFmpeg device specification."
        )
    ...
    if not device_list:
        return (
            "No input devices detected. Provide an FFmpeg capture specification "
            "such as 'pulse:default?sample_rate=48000&channels=2'."
        )
